...
| `contractMode` | string | no | Set to `"basic"` on contracts generated by the Basic wizard, so `npx seo-setup` can detect and re-edit them. Not validated or used elsewhere — safe to omit for Custom contracts. See [Basic mode contracts](#basic-mode-contracts-mode-basic). |
...
## Basic mode contracts (`mode: "basic"`)

Contracts generated by `npx seo-setup` → **Basic** use a different check shape: existence / minimum-length instead of an exact `value`. Six checks support it — `metadata.title`, `metadata.metaDescription`, `metadata.h1`, `metadata.canonical`, `metadata.hreflang`, and `httpChecks.xRobotsTag` (paired with `metadata.metaRobots` for indexability):
...
|---|---|---|
| `mode: "basic"` | any of the six checks above | Switches the check from exact-`value` matching to existence / minimum-length semantics. Omitting `mode` (or any value other than `"basic"`) keeps the normal exact-match behaviour. `"basic"` is currently the only supported mode. |
| `minLength` | `title`, `metaDescription`, `h1` | The tag must exist exactly once and its trimmed text must be longer than this many characters. For `h1`, length is measured on `textContent` (not `innerText`), so a logo-as-H1 pattern like `<h1><a><svg><title>Site Name</title>...</svg></a></h1>` is measured correctly instead of reading as empty. |

Basic-mode behaviour per check:

- **`title` / `metaDescription`**: exactly one tag must exist, with content longer than `minLength`.
- **`h1`**: exactly one `<h1>` must exist (same "exactly one" check as Custom mode), with `textContent` longer than `minLength`.
- **`canonical`**: exactly one `<link rel="canonical">` must exist with a non-empty `href`. The raw-HTML "Canonical Misdirection Trap" check is skipped in Basic mode since it requires a known expected URL.
- **`metaRobots` + `xRobotsTag`**: the page is indexable — neither the `meta[name="robots"]` content nor the `X-Robots-Tag` header may contain `noindex`. The wizard presents this as a single "Indexability" toggle and writes both keys together.
- **`hreflang`**: at least **two** `link[rel="alternate"][hreflang]` tags must be present. A valid hreflang implementation is reciprocal — the page must reference itself plus at least one alternate — so a single tag already indicates an incomplete setup.

A Basic contract also sets a top-level `"contractMode": "basic"` marker so `npx seo-setup` can detect it on re-run and offer to edit it directly, instead of routing you through the Custom CSV/example flow. See [seo-checks.basic.example.json](../seo-checks.basic.example.json) for a full generated example, and the README's [Quick start](../README.md#3-create-your-seo-contract-seo-checksjson) for how the wizard builds one (auto-crawling `sitemap.xml`, a pasted list of paths, or a simple CSV).
...
**Switching modes.** Re-running `npx seo-setup` against an existing contract offers a switch in either direction: a Basic contract gets a "Switch to Custom" option (routes into the normal CSV/example flow), and a Custom contract gets a "Switch to Basic" option (routes into the Basic wizard). Either direction **replaces** the existing `seo-checks.json` with a freshly built one for the new mode — nothing is merged or carried over between them, so confirm that's what you want before switching.
